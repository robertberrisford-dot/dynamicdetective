...
import { ArrowLeft, AlertCircle, CheckCircle2, Clock, Globe, Link2, FileWarning, Type, ChevronRight, Users, Hash, Search, Filter, ChevronDown, Copy, ExternalLink, AlertTriangle, ShieldAlert, ClipboardCheck, Info } from 'lucide-react';
...
const ISSUE_TYPE_CONFIG: Record<string, { label: string; icon: typeof AlertCircle; color: string; bgColor: string; severity: Severity }> = {
  ...
  automatic_source_review: { label: 'Automatic Source Review', icon: AlertTriangle, color: 'text-amber-600', bgColor: 'bg-amber-50 dark:bg-amber-950/30', severity: 'warning' },
};
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      {issue.issue_type === 'automatic_source_review' && issue.voucher_code && (
                        <div className="mt-1 flex items-center gap-1">
                          ...
                          <Button variant="ghost" size="icon" className="h-5 w-5" onClick={(e) => { e.stopPropagation(); navigator.clipboard.writeText(issue.voucher_code!); toast.success('Code copied'); }}>
                      ...
                      {issue.seo_url && issue.issue_type !== 'wrong_country_redirect_url' && (
                        <Button variant="ghost" size="sm" className="h-6 gap-1 px-2 text-[10px] text-primary hover:text-primary mt-1" onClick={(e) => { e.stopPropagation(); const base = (country === 'pl') ? 'https://www.pepper.pl/kupony' : (country === 'uk') ? 'https://www.hotukdeals.com/vouchers' : 'https://www.mydealz.de/gutscheine'; window.open(`${base}/${issue.seo_url}`, '_blank', 'noopener,noreferrer'); }}>
