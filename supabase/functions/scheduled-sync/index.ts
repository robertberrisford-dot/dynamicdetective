...
Deno.serve(async (req) => {
  ...
  const mode = body.mode || "full"; // "full" = sheet sync + URL check, "url-only" = just URL check
  ...
  // --- 1. Sheet Sync (only in "full" mode) ---
  if (mode === "full") {
    ...
    try {
      const syncRes = await fetch(`${supabaseUrl}/functions/v1/sync-google-sheet`, {
        ...
        body: JSON.stringify({ spreadsheet_id: SPREADSHEET_ID, sheet_name: SHEET_NAME }),
      });
      ...
      await adminClient.from("sync_logs").update({ status: "error", message, finished_at: new Date().toISOString() }).eq("id", syncLogId);
      results.push({ function_name: "sync-google-sheet", status: "error", message, details: null });
    }
  }

  // --- 2. URL Check — single batch ---
  const batchId = `scheduled-${new Date().toISOString().slice(0, 10)}`;

  // Check if today's URL check is already complete
  ...
  if (todayLogs && todayLogs.length > 0) {
    ...
    try {
      const urlRes = await fetch(`${supabaseUrl}/functions/v1/check-urls`, {
        ...
        body: JSON.stringify({ spreadsheet_id: SPREADSHEET_ID, sheet_name: SHEET_NAME, batch_id: batchId }),
      });
