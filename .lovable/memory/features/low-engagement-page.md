...
description: Flags dead vouchers on pages where >40% of vouchers have CPD<0.2 (7d); CPD from column AK voucher_rank_cpd
type: feature
---
...
- Trigger per retailer landing page (grouped by `retailer_pool_id`):
  ...
  - `> 40%` of them have parseable CPD `< 0.2`
- Emits one issue per dead voucher with description `[Code|Deal] Low engagement: X/Y (Z%) on {client_name} have CPD<0.2 (7d). This voucher CPD: N.NN.`
